...
CREATE PROCEDURE [dbo].[sp_proc_Dwn_Monitor] AS 
    ...
    SET @Max_Job = (SELECT Parm_Value FROM dbo.ref_Parameters WHERE Parm_Name = 'Max_Dwn_Jobs')
    SET @Active_Jobs = (SELECT COUNT(*) FROM vw_UTIL_SSIS_Active_Summary WHERE package_name LIKE 'dwn_%')
    ...
    SELECT TOP (' + CAST(@Available_Slots AS NVARCHAR(10)) + N') tbl_Key, dwn_Pkg
    ...
    SELECT tbl_Key, dwn_Pkg FROM #tempTable
	...
	For Select Top @Available_Slots tbl_Key
	, dwn_Pkg
	From ref_Table_Dictionary 
	Where dwn_Flag = 'R'
	and Active = 'Y'
