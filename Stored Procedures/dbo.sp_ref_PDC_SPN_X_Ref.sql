...
CREATE PROCEDURE [dbo].[sp_ref_PDC_SPN_X_Ref]  AS
...
Select CORE_PN
  ...
  On CORE_PN=epm.PN
  Where (Rev<80 Or (Rev>99 And Rev<300))
...
--Correct SPN for PNs with Package Quantity Suffixes to match the SPN of the corresponding base PNs.

With s As
(
Select SPN
	  ,PN
	  ,Left(PN,8) As Base_PN
From dbo.ref_PDC_SPN_X_Ref
Where PN Like ' % [0-9]%'
)
, u As
...
Select s.SPN
	  ,s.PN
	  ,Base_PN
	  ,x.SPN As Updated_SPN
From s
Join dbo.ref_PDC_SPN_X_Ref x
On s.Base_PN=x.PN
Where Base_PN=' 1328692'
)
Select Distinct *
Into #u
From u
...
--Remove bad records coming up in the audits.
Delete
From dbo.ref_PDC_SPN_X_Ref
Where (PN='Y518804689  ' And SPN='1370197')
;
