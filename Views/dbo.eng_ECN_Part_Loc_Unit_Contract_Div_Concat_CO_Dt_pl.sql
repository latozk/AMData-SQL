...
CREATE view [dbo].[eng_ECN_Part_Loc_Unit_Contract_Div_Concat_CO_Dt_pl] as 
...
WHERE t1.[TRAN_TYPE]='PM' AND (t1.[TRAN]='ADD' OR t1.[TRAN]='REL')
)
...
SELECT DISTINCT f2.ECN, f2.PN, f2.Loc, Unit= 
    STUFF((SELECT  ', ' + ' '+Unit+'-'+RTrim(Contract)+'-'+Div
           FROM t0 f1 
           WHERE f1.ECN = F2.ECN AND f1.PN = f2.PN AND f1.Loc = f2.Loc
          FOR XML PATH('')), 1, 2, '')
FROM t0 f2
